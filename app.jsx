...
const MenuEntries = require('./menuItems.jsx'); // Import the Product model

// API route for uploading product
app.post('/api/upload', async (req, res) => {
  try {
    // Validate required fields
    const { id, name, image, rating, price, type, isVeg } = req.body;
    if (!id || !name || !image || rating === undefined || price === undefined || !type || isVeg === undefined) {
      return res.status(400).send('All fields are required');
    }

    // Convert rating and price to numbers
    const MenuEntriesData = {
      id,
      name,
      image,
    ...
    const prodMenuEntries = new MenuEntries(MenuEntriesData); // Use MenuEntries to create a new instance

    // Save the product to the database
    await prodMenuEntries.save(); // Save the newly created instance
